...
In this codebase, prefer importing Motion from `motion/react` (same component API as classic Framer Motion).
...
11. **Prefer spring physics for organic motion.** Springs have natural overshoot and settle. Use spring configs in Motion (e.g. `type: "spring", stiffness, damping`) or CSS `linear()` / cubic-bezier approximations.
